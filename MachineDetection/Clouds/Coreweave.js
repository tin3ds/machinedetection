...
class Coreweave {
    static async getMetadata() {
        ...
        try {
            ...
            const data = fs.readFileSync(path.join(os.userInfo().homedir, "Desktop", "vm-name.txt"), "utf8");
            ...
            if(err && err.message && typeof err.message=="string" && err.message.length && err.message.startsWith(`Request timed out after`)){
                let result = {};
                result[`instance-id`] = vmName;
                result[`region`] = null;
                result[`name`] = vmName;
